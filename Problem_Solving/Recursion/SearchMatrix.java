public class SearchMatrix {
    ...
    public boolean searchMatrix(int[][] matrix, int target) {
        return backtracking(matrix, 0, matrix[0].length - 1, target);
    }

    public boolean backtracking(int[][] matrix, int row, int col, int target) {
        ...
        if (matrix[row][col] < target) {
            return backtracking(matrix, row + 1, col, target);
        } else {
            return backtracking(matrix, row, col - 1, target);
        }
    }
